...
setwd("~/Documents/Imperial/Morphological Character Hierarchies/Data/Rose et al, 2014/")
library(ape)
library(distory)
library(phangorn)
library(phytools)
...
# paup
pauptrees <- read.nexus("Pauptrees-R2014.tre")
paup <- read.nexus("Paupcontree-R2014.tre")
paup <- unroot(paup)
pauptrees <- as.multiPhylo(pauptrees)
pauptrees <- unroot.multiPhylo(pauptrees)

# morphy
morphytrees <- read.nexus("Morphytrees-R2014.tre")
morphy <- read.nexus("Morphycontree-R2014.tre")
...
write.csv(rawdata, file = "rawdataR2014.csv")
...
write.csv(normdata, file = "normdataR2014.csv")
...
### RF
# paup-morphy
wilcox.test(pprf, mmrf) # W = 642, p-value = 0.03927
# mmrf > pprf

### Quartet
# paup-morphy
wilcox.test(ppqu, mmqu) # W = 348, p-value = 0.007228
# mmqu > ppqu
...
pdf("Figures/RF MDS with density distribution R2014.pdf", 8, 5.3)
ggdraw() +
...
mdsquplot3 <- ggplot(mdsqupoints) +
  ...
  theme(legend.position = "none", axis.title = element_text(size = 11)) +
  fill_palette(palette = c(palette[2], palette[1])) +
  scale_x_continuous(breaks = c(0, 0.04))
...
pdf("Figures/Quartet MDS with density distribution R2014.pdf", 8, 5.3)
ggdraw() +
