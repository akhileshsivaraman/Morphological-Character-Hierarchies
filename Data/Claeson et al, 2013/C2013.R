...
write.csv(rawdata, file = "rawdataC2013.csv")
...
write.csv(normdata, file = "normdataC2013.csv")
...
bhatt.coeff(normpprf, normpmrf) # 0.6619211
bhatt.coeff(normpprf, normparf) # 0.8402228
bhatt.coeff(normpmrf, normparf) # 0.9443761
bhatt.coeff(normpmrf, normmarf) # 0.9368992
bhatt.coeff(normparf, normmarf) # 0.9318233
...
bhatt.coeff(normppqu, normpmqu) # 0.4925949
bhatt.coeff(normppqu, normpaqu) # 0.8342182
bhatt.coeff(normpmqu, normpaqu) # 0.8568308
bhatt.coeff(normpmqu, normmaqu) # 0.9276204
bhatt.coeff(normpaqu, normmaqu) # 0.799762
...
### RF
# paup-morphy
t.test(pprf, mmrf) # t = -62.145, df = 19502, p-value < 2.2e-16
# mmrf > pprf

# paup-ana
t.test(pprf, aarf) # t = -56.698, df = 16636, p-value < 2.2e-16
# aarf > pprf

# morphy-ana
t.test(mmrf, aarf) # t = 1.9869, df = 17914, p-value = 0.04695
# mmrf > aarf

### Quartet
# paup-morphy
t.test(ppqu, mmqu) # t = -94.649, df = 14990, p-value < 2.2e-16
# mmqu > ppqu

# paup-ana
t.test(ppqu, aaqu) # t = -67.949, df = 11838, p-value < 2.2e-16
# aaqu > ppqu

# morphy-ana
t.test(mmqu, aaqu) # t = 13.829, df = 17606, p-value < 2.2e-16
# mmqu > aaqu
...
pdf("Figures/RF MDS with density distribution C2013.pdf", 8, 5.3)
ggdraw() +
...
# Scatter plot with marginal density distribution plot
jitter <- position_jitter(width = 200, height = 900)
...
mdsquplot3 <- ggplot(mdsqupoints) +
  ...
  theme(legend.position = "none", axis.title = element_text(size = 11)) +
  fill_palette(palette = c(palette[3], palette[2], palette[1])) +
  scale_x_continuous(breaks = c(0, 0.004))
...
pdf("Figures/Quartet MDS with density distribution C2013.pdf", 8, 5.3)
ggdraw() +
...
d <- as.data.frame(a$pco$li)
d1 <- cbind(d, Programme)
medTree(treeslist)
