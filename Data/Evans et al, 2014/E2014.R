...
# paup morphy
pmrf <- rf[1:100, 101:193]
pmrf <- as.matrix(pmrf)
pmrf <- as.vector(pmrf)

# paup anagallis
parf <- rf[1:100, 194:203]
parf <- as.matrix(parf)
parf <- as.vector(parf)

# morphy anagallis
marf <- rf[101:193, 194:203]
marf <- as.matrix(marf)
marf <- as.vector(marf)
...
# paup-anagallis
paqu <- qu4[1:100, 194:203]
paqu <- as.matrix(paqu)
paqu <- as.vector(paqu)
paququartiles <- quantile(paqu)

# morphy-anagallis
maqu <- qu4[101:193, 194:203]
maqu <- as.matrix(maqu)
maqu <- as.vector(maqu)
...
write.csv(rawdata, file = "rawdataE2014.csv")
...
write.csv(normdata, file = "normdataE2014.csv")
...
bhatt.coeff(normpmrf, normparf) # 0.6728658
bhatt.coeff(normpmrf, normmarf) # 0.8357164
bhatt.coeff(normparf, normmarf) # 0.4599974
...
bhatt.coeff(normpmqu, normpaqu) # 0.5366944
bhatt.coeff(normpmqu, normmaqu) # 0.6669199
bhatt.coeff(normpaqu, normmaqu) # 0.4028775
...
### RF
# paup-morphy
wilcox.test(pprf, mmrf) # W = 5280480, p-value < 2.2e-16
# mmrf > pprf

# paup-ana
wilcox.test(pprf, aarf) # W = 480442, p-value = 0.4964
# neither larger

# morphy-ana
wilcox.test(mmrf, aarf) # W = 758127, p-value < 2.2e-16
# mmrf > aarf

### Quartet
# paup-morphy
wilcox.test(ppqu, mmqu) # W = 24954866, p-value < 2.2e-16
# mmqu > ppqu

# paup-ana
wilcox.test(ppqu, aaqu) # W = 663612, p-value = 1.707e-08
# ppqu > aaqu 

# morphy-ana
wilcox.test(mmqu, aaqu) # W = 740587, p-value < 2.2e-16
# mmqu > aaqu
...
pdf("Figures/RF MDS with density distribution E2014.pdf", 8, 5.3)
ggdraw() +
...
mdsquplot3 <- ggplot(mdsqupoints) +
  ...
  theme(legend.position = "none", axis.title = element_text(size = 11)) +
  fill_palette(palette = c(palette[3], palette[2], palette[1])) +
  scale_x_continuous(breaks = c(0,0.0025))
...
pdf("Figures/Quartet MDS with density distribution E2014.pdf", 8, 5.3)
ggdraw() +
...
#### Tree Contradiction - doesn't work, leave out #### 
# generate tree contradiction ma- doesntrix
tc <- sapply(treeslist, function(x)
  sapply(treeslist, treeContradiction, tree1=x))

colnames(tc) <- rownames(tc) <- c(paste("Paup", 1:length(pauptrees), sep = ""), 
                                  paste("Morphy", 1:length(morphytrees), sep = ""),
                                  paste("Anagallis", 1:length(anatrees), sep = ""))
tc <- as.data.frame(tc)
...
# paup-morphy
pmtc <- tc[1:100, 101:193]
pmtc <- as.matrix(pmtc)
pmtc <- as.vector(pmtc)

# paup-anagallis
patc <- tc[1:100, 194:203]
patc <- as.matrix(patc)
patc <- as.vector(patc)

# morphy-anagallis
matc <- tc[101:193, 194:203]
matc <- as.matrix(matc)
matc <- as.vector(matc)
...
### Tree Contradiction
mdstc <- metaMDS(tc, distance = "euclidean", trymax = 999)
stressplot(mdstc)
mdstcpoints <- as.data.frame(mdstc$points)
mdstcpoints <- cbind(mdstcpoints, Programme)

jitter <- position_jitter(width = 0.6, height = 0.6)

# scatter plot with marginal density distribution plot
mdstcplot1 <- ggplot(mdstcpoints) +
  geom_point(aes(MDS1, MDS2, colour = Programme), position = jitter, size = 2.5, alpha = 1) +
  theme_pubr() + 
  theme(axis.line = element_blank(), axis.ticks = element_blank(), axis.title = element_blank(), 
        axis.text = element_blank(), legend.position = "none", panel.border = element_rect(colour = "grey", size = 0.5, fill = NA)) +
...
mdstcplot2 <- ggplot(mdstcpoints) +
  geom_density(aes(MDS1, fill = Programme), alpha = 0.4) +
  theme_pubr() +
  theme(axis.title = element_text(size = 11)) +
  fill_palette(palette = c(palette[3], palette[2], palette[1]))

mdstcplot3 <- ggplot(mdstcpoints) +
...
pdf("Figures/TC MDS with density distribution E2014.pdf", 8, 5.3)
ggdraw() +
  draw_plot(mdstcplot2, x = 0, y = 0.7, width = 0.7, height = 0.3) +
  draw_plot(mdstcplot1, x = 0, y = 0, width = 0.7, height = 0.7) +
  draw_plot(mdstcplot3, x = 0.7, y = 0, width = 0.3, height = 0.7) +
  draw_plot_label(label = "A", x = 0, y = 0.98) +
...
pdf("Figures/TC MDS with ellipse E2014.pdf", 8, 5.3)
ggplot(mdstcpoints) +
  geom_point(aes(MDS1, MDS2, colour = Programme), size = 2.5) +
  stat_ellipse(aes(MDS1, MDS2, color = Programme, fill = Programme), alpha = 0.05, geom = "polygon") +
  color_palette(palette = c(palette[3], palette[2], palette[1])) +
  fill_palette(palette = c(palette[3], palette[2], palette[1])) +
  theme_pubr() +
