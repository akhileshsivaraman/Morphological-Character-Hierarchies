...
# paup anagallis
parf <- rf[1:100, 201:210]
parf <- as.matrix(parf)
parf <- as.vector(parf)

# morphy anagallis
marf <- rf[101:200, 201:210]
marf <- as.matrix(marf)
marf <- as.vector(marf)
...
write.csv(rawdata, file = "rawdatadL2020.csv")
...
write.csv(normdata, file = "normdatadL2020.csv")
...
bhatt.coeff(normpmrf, normparf) # 0.5392036
bhatt.coeff(normpmrf, normmarf) # 0.8868167
bhatt.coeff(normparf, normmarf) # 0.817147
...
bhatt.coeff(normpmqu, normpaqu) # 0.2001918
bhatt.coeff(normpmqu, normmaqu) # 0.402723
bhatt.coeff(normpaqu, normmaqu) # 0
...
### RF
# paup-morphy
wilcox.test(pprf, mmrf) # W = 6185834, p-value < 2.2e-16
# mmrf > pprf

# paup-ana
wilcox.test(pprf, aarf) # W = 322010, p-value = 3.897e-10
# aarf > pprf

# morphy-ana
wilcox.test(mmrf, aarf) # W = 855736, p-value < 2.2e-16
# mmrf > aarf


### Quartet
# paup-morphy
wilcox.test(ppqu, mmqu) # W = 13454876, p-value < 2.2e-16
# mmqu > ppqu

# paup-ana
wilcox.test(ppqu, aaqu) # W = 441174, p-value = 0.0426
# aaqu > ppqu

# morphy-ana
wilcox.test(mmqu, aaqu) # W = 861248, p-value < 2.2e-16
# mmqu > aaqu
...
pdf("Figures/RF MDS with density distribution dL2020.pdf", 8, 5.3)
ggdraw() +
