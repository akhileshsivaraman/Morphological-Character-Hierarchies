...
# paup anagallis
parf <- rf[1:100, 201:246]
parf <- as.matrix(parf)
parf <- as.vector(parf)

# morphy anagallis
marf <- rf[101:200, 201:246]
marf <- as.matrix(marf)
marf <- as.vector(marf)
...
# paup-anagallis
paqu <- qu4[1:100, 201:246]
paqu <- as.matrix(paqu)
paqu <- as.vector(paqu)
paququartiles <- quantile(paqu)

# morphy-anagallis
maqu <- qu4[101:200, 201:246]
maqu <- as.matrix(maqu)
maqu <- as.vector(maqu)
...
write.csv(rawdata, file = "rawdataL2020.csv")
...
write.csv(normdata, file = "normdataL2020.csv")
...
bhatt.coeff(normpprf, normpmrf) # 0.6366406
bhatt.coeff(normpprf, normparf) # 0.4502684
bhatt.coeff(normpmrf, normparf) # 0.9283584
bhatt.coeff(normpmrf, normmarf) # 0.9789234
bhatt.coeff(normparf, normmarf) # 0.9580702
...
bhatt.coeff(normppqu, normpmqu) # 0.6537421
bhatt.coeff(normppqu, normpaqu) # 0.4785662
bhatt.coeff(normpmqu, normpaqu) # 0.9219769
bhatt.coeff(normpmqu, normmaqu) # 0.9757264
bhatt.coeff(normpaqu, normmaqu) # 0.962573
...
### RF
# paup-morphy
t.test(pprf, mmrf) # t = -36.934, df = 2864.4, p-value < 2.2e-16
# mrf > pprf

# paup-ana
t.test(pprf, aarf) # t = -4.1242, df = 63.504, p-value = 0.0001101
# aarf > pprf 

# morphy-ana
t.test(mmrf, aarf) # t = 2.0109, df = 66.045, p-value = 0.04842


### Quartet
# paup-morphy
t.test(ppqu, mmqu) # t = -88.011, df = 19511, p-value < 2.2e-16
# mmqu > ppqu

# paup-ana
t.test(ppqu, aaqu) # t = -48.261, df = 2501.8, p-value < 2.2e-16
# aaqu > ppqu

# morphy-ana
t.test(mmqu, aaqu) # t = -9.4401, df = 2652.9, p-value < 2.2e-16
# aaqu > mmqu
...
pdf("Figures/RF MDS with density distribution L2020.pdf", 8, 5.3)
ggdraw() +
...
mdsquplot3 <- ggplot(mdsqupoints) +
  ...
  theme(legend.position = "none", axis.title = element_text(size = 11)) +
  fill_palette(palette = c(palette[3], palette[2], palette[1])) +
  scale_x_continuous(breaks = c(0, 0.0003))
...
pdf("Figures/Quartet MDS with density distribution L2020.pdf", 8, 5.3)
ggdraw() +
